...
param (
    ...
    [string]$OutputDir = "$env:AppData\Export-Permission\Logs",
...
begin {
    ...
    $OutputDir = New-DatedSubfolder -Root $OutputDir
...
process {
    ...
    ForEach ($ThisTargetPath in $TargetPath) {
        ...
        Write-LogMsg @LogParams -Text "Expand-Folder -FolderTargets @('$($FolderTargets -join "',")') -LevelsOfSubfolders $SubfolderLevels"
        $Subfolders = Expand-Folder -FolderTargets $FolderTargets -LevelsOfSubfolders $SubfolderLevels @LoggingParams

        Write-LogMsg @LogParams -Text "Get-FolderAccessList -FolderTargets @('$($Subfolders -join "',")')"
        $Permissions = Get-FolderAccessList -Folder $FolderTargets -Subfolder $Subfolders -ThreadCount $ThreadCount @LoggingParams
        ...
        if ($ExcludeAccount) {
            ...
            Write-LogMsg @LogParams -Text "New-BootstrapDivWithHeading -HeadingText 'Output Folder: $OutputDir' -Content `"`$Description`$ListGroup`""
        ...
        if ($NoGroupMembers) {
            $Description = 'Group memberships were not evaluated.<br />Only accounts directly from the ACLs are included in the report.'
        } else {
            $Description = 'No group members were excluded.<br />Members of groups with access are included in the report.'
        }
        $HtmlExcludedGroupMembers = New-BootstrapDivWithHeading -HeadingText $HeadingText -Content $Description

        Write-LogMsg @LogParams -Text "New-BootstrapColumn -Html 'Output Folder: `$HtmlListOfReports',`$HtmlListOfLogs"
        ...
        Write-LogMsg @LogParams -Text "New-BootstrapDivWithHeading -HeadingText 'Output Folder: $OutputDir' -Content `$FileListColumns"
        $FileList = New-BootstrapDivWithHeading -HeadingText "Output Folder: $OutputDir" -Content $FileListColumns

        Write-LogMsg @LogParams -Text "Get-ReportFooter -StopWatch `$StopWatch -WhoAmI '$WhoAmI' -ThisFqdn '$ThisFqdn'"
        $ReportFooter = Get-HtmlReportFooter -StopWatch $StopWatch -WhoAmI $WhoAmI -ThisFqdn $ThisFqdn -ItemCount ($Subfolders.Count + $FolderTargets.Count)
